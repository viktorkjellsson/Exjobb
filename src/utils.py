...
def calculate_anomalous_regions_likelihood(original, reconstructed, n=5, p_threshold=0.01):
    error = original - reconstructed
    mu = np.mean(error)
    sigma = np.std(error)
    sigma = max(sigma, 1e-6)
    p_values = 2 * (1 - norm.cdf(np.abs(error), loc=mu, scale=sigma))
    anomalous_points = p_values < p_threshold
    ...
    for start, end in regions:
        final_indices.extend(range(start, end + 1))

    return final_indices, p_values

def plot_reconstruction_likelihood(dataset, model, N, feature_names, timestamps, p_threshold=0.01, ncol=1):
    model.eval()
    ...
    num_features = data_subset.shape[1]
    ...
    for i, ax in enumerate(axes):
        if i < num_features:
            ...
            # p-value based anomaly detection
            anomalous_indices, p_values = calculate_anomalous_regions_likelihood(
            data_subset[:, i], reconstructed[:, i], p_threshold=p_threshold)

            df_anomalies = sort_anomalies(anomalous_indices, timestamps)
            print(f'\n{feature_name} Anomalies:\n{df_anomalies}')

            """Convert p-values to scaled anomaly scores (higher = more anomalous)."""
            # Avoid log(0) by clipping
            p_values = np.clip(p_values, 1e-10, 1.0)
            log_p_values = -np.log10(p_values)

            scaler = MinMaxScaler()
            scaled_scores = scaler.fit_transform(log_p_values.reshape(-1, 1)).flatten()
            log_p_values_scaled = 1 - scaled_scores  # High anomaly score closer to 1

            ax.plot(timestamps, data_subset[:, i], label="True", alpha=0.8)
            ax.plot(timestamps, reconstructed[:, i], label="Reconstructed", alpha=0.8)
            ax.plot(timestamps, log_p_values_scaled, label="Anomaly Score", alpha=0.8, color='green')

            ax.fill_between(timestamps, 0, 1,
                    where=np.isin(np.arange(len(timestamps)), anomalous_indices),
